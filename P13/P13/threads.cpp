...
//	void find_primes_theads(int num_threads, int i, int max){
//	  int starts=2, perthread;
//	  perthread=(max-starts)/num_threads;
//	  find_primes( 2,i*perthread);	  
//	}
	void find_prime_threads(int min,int range){
		find_primes(min,min+range);
	}
...
int main(int argc, char* argv[]) {
	...
	std::thread t[threads];
		int start = 2;
		int range = (max_int-2)/threads;
	for(int i = 0; i<threads;i++){
		int threads_start = start;
		t[i] = std::thread(&Prime_numbers::find_prime_threads,&find_primes,threads_start,range);
